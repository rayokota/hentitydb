...
import io.hentitydb.store.hbase.HBaseColumn;
import io.hentitydb.store.hbase.HBaseCompactor;
import io.hentitydb.store.hbase.HBaseConnectionFactory;
import io.hentitydb.store.hbase.HBaseDelete;
...
public class MockHTable implements Table {
    ...
    public <K, C> void majorCompact(TableMetadata<K, C> metadata) {
        try {
            ...
            props = HBaseConnectionFactory.addCompactionProps(metadata, props);
            for (String columnFamily : columnFamilies) {
                majorCompact(metadata, columnFamily, props);
            }
        } catch (IOException ioe) {
            throw new RuntimeException("Could not compact table: " + tableName, ioe);
        }
    }
    ...
    private <K, C> void majorCompact(TableMetadata<K, C> metadata, String family, Map<String, String> props) throws IOException {
        ...
        final Codec<K> keyCodec = (Codec<K>) HBaseCompactor.getKeyCodecInstance(props, CompactionFilter.HENTITYDB_PREFIX + "." + CompactionFilter.KEY_CODEC);
        if (keyCodec == null) {
            // we allow the keyCodec to be null; in this case the code will use the raw key
            System.out.println("WARNING: No key codec for: " + tableName + ", " + family);
        }

        final Codec<C> columnCodec = (Codec<C>) HBaseCompactor.getInstance(props, CompactionFilter.HENTITYDB_PREFIX + "." + CompactionFilter.COLUMN_CODEC);
        if (columnCodec == null) {
            System.out.println("ERROR: No column codec for: " + tableName + ", " + family);
            return;
        }
        ...
        while (iter.hasNext()) {
            ...
            for (Cell cell : result.listCells()) {
                ...
                boolean doFilter = filter.filterKeyColumn(keyColumn);
                if (!doFilter) {
                    doDelete = true;
                    deleteForFilter.addColumns(Bytes.toBytes(family), CellUtil.cloneQualifier(cell), cell.getTimestamp());
                }
                ...
                if (delete != null && delete.hasColumns()) {
                    delete(((HBaseDelete<K, C>) delete).getHOperation());
                }
            }
            if (doDelete) delete(deleteForFilter);
        }
